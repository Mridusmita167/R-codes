...
CD <- read.csv("F:/excelr/Assignment/Decision Tree/Company_Data.csv")
View(CD)
colnames(CD)
hist(CD$Sales)
...
train <- CompanyData[1:320,]
test <- CompanyData[321:400,]

# Building the model with 'ctree' function from 'party' package

library(party)
tree <- ctree(High ~., data = train)
plot(tree)
...
mean(pred == test$High) # Accuracy = 85%

# Cross Table

library(gmodels)
CrossTable(test$High, pred)
